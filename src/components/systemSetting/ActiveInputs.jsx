            ...
            <div className='flex  flex-col gap-5 overflow-auto h-[65vh]'>
                ...
                {inputs?.length > 0 ? inputs.map((input, index) => (
                    <div title={hoverTitle[input?.label]} key={input?._id} onClick={() => setInputDeatils(input)} className='flex w-5/6 pt-4  gap-2'>
                        ...
                        {input?.type === "short" && <ShortInput setFunc={() => input?.label !== "מ.א של לקוח" ? setChooseOption(`short_input_${input?._id}`) : setChooseOption("")} defaultValue={input?.placeholder || "הקלידו"} title={input?.label} required={input?.require} chooseOption={chooseOption === `short_input_${input?._id}`} />}
                    </div>
